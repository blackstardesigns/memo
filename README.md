...
| `h` | Toggle the notes/folders drawer (a tree on the left) |
| `d` | Delete the selected note or folder (asks for confirmation) |
| `x` | Export the selected note as a `.md` file |
| `?` | Show help |
...
Press `h` on the home screen to toggle a drawer on the left that shows your whole notes-and-folders hierarchy as a tree. It scrolls independently of the main view. With the drawer focused, navigate with the arrow keys:
